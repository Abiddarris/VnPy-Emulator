...
public class ExceptFinally {
    ...
    List<Except> exceptionHandlers = new ArrayList<>();
    Runnable tryRunnable;

    public ExceptFinally(Runnable tryRunnable) {
        this.tryRunnable = tryRunnable;
    }
    
    public ExceptFinallyElseExecutable onExcept(ExceptionHandler handler, PythonObject... exceptionsType) {
        exceptionHandlers.add(new Except(handler, exceptionsType));
        
        return new ExceptFinallyElseExecutable(this);
    }
