...
public class PythonSDLActivity extends SDLActivity {
    ...
    public void preparePython() {
        ...
        String gameScript = arguments.getGameScript();

        if (gameScript.equalsIgnoreCase("main.py")) {
            String newName = "_main.py";
            originalScript = new File(path, newName);
            if (!new File(path, gameScript).renameTo(originalScript)) {
                throw new UncheckedIOException(new IOException("Cannot rename main.py"));
            }
        }

        mainPy = new File(path, "main.py");
        try (InputStream in = getAssets().open("main.py");
             OutputStream out = Files.openBufferedOutput(mainPy)){
            InputStreams.writeAllTo(in, out);
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }

        nativeSetEnv("GAME_SCRIPT", gameScript);
        nativeSetEnv("ANDROID_PRIVATE", path);
    ...
    protected void onDestroy() {
        ...
        if (mainPy != null) {
            ...
            if (originalScript != null) {
                originalScript.renameTo(mainPy);
            }
        }
        super.onDestroy();
    }
